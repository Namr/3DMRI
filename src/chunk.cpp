...
void Chunk::init() {
  ...
  model = glm::translate(
      model, glm::vec3((float)chunkX, (float)chunkY, (float)chunkZ));
  ...
  glBufferData(GL_ARRAY_BUFFER, newVertices.size() * sizeof(Vertex),
               &newVertices[0], GL_STATIC_DRAW);

  // pass and bind triangle data
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
  glBufferData(GL_ELEMENT_ARRAY_BUFFER,
               newTriangles.size() * sizeof(unsigned int), &newTriangles[0],
               GL_STATIC_DRAW);
  ...
  glVertexAttribPointer(colAttrib, 1, GL_FLOAT, GL_FALSE, sizeof(Vertex),
...
void Chunk::update(Camera &camera) {
  ...
  glUniformMatrix3fv(glGetUniformLocation(shaderProgram, "wireColor"), 1,
                     GL_FALSE, glm::value_ptr(wcolor));
